...
#define dout_subsys ceph_subsys_ecutil
#undef dout_prefix
...
void ECUtil::CompactInfo::decompact(uint8_t shard, uint32_t offset, uint32_t len, const bufferlist& src, bufferlist& dst)
{
    ...
    for (uint32_t step = 0; step < src.length(); step += decode_step) {
      ...
      ldout(g_ceph_context, 20) << __func__ << " shard " << shard << " step " << step << " decode_step "
