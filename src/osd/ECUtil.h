...
public:
  ...
  pair<uint32_t, uint32_t> convert_compact_ranges(uint8_t shard,
    ...
    uint32_t end_chunk = (offset + len) / chunk_size - 1;
  ...
  void decompact(uint8_t shard, uint32_t offset, uint32_t len, const bufferlist& src, bufferlist& dst);

  uint64_t get_total_chunk_size(uint8_t shard) const {
    return get_chunk_compact_range(shard).back();
  }

  pair<uint32_t, uint32_t> reverse_convert_compact_ranges(uint8_t shard,
    uint32_t offset, uint32_t len) {
    uint32_t begin_chunk = conver_compact_range(shard, offset);
    return make_pair(begin_chunk * chunk_size,
      (conver_compact_range(shard, offset + len) - begin_chunk) * chunk_size);
  }
