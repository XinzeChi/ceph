...
TEST(BufferPool, test)
{
  ...
  {
    auto ret = pool->get_from_pool(raw_size, buffer::raw_type::ALIGN); 
    EXPECT_EQ(NULL, ret);
  }
  ...
  {
    buffer::raw *raw = pool->get_from_pool(raw_size, buffer::raw_type::ALIGN);
    EXPECT_TRUE(raw != NULL);
    bufferptr ptr = raw;
    raw = pool->get_from_pool(raw_size, buffer::raw_type::ALIGN);
    EXPECT_TRUE(raw == NULL);
  }

  {
    bufferptr ptr = pool->get_from_pool(raw_size, buffer::raw_type::ALIGN);
    EXPECT_TRUE(ptr.get_raw() != NULL);
  }
  ...
  {
    for (int i = 0; i < 3; i++) {
      buffer::raw *raw = pool->get_from_pool(raw_size + 1, buffer::raw_type::NORMAL);
      EXPECT_TRUE(raw != NULL);
    }
    buffer::raw *raw = pool->get_from_pool(raw_size + 1, buffer::raw_type::NORMAL);
    EXPECT_TRUE(raw == NULL);
    raw = pool->get_from_pool(raw_size + 1, buffer::raw_type::ALIGN);
    EXPECT_TRUE(raw == NULL);
  }
  delete pool;
}
