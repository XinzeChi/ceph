...
class PrioritizedQueue {
  ...
  struct SubQueue {
  private:
    ceph::unordered_map<K, list<pair<unsigned, T> > > q;
    unsigned tokens, max_tokens;
    int64_t size;
    K cur_key;
    typename ceph::unordered_map<K, list<pair<unsigned, T> > >::iterator cur;
  public:
    ...
    void remove_by_filter(F f, list<T> *out) {
      for (typename ceph::unordered_map<K, list<pair<unsigned, T> > >::iterator i = q.begin();
	   i != q.end();
	   ) {
    ...
    void remove_by_class(K k, list<T> *out) {
      typename ceph::unordered_map<K, list<pair<unsigned, T> > >::iterator i = q.find(k);
      if (i == q.end())
	return;
      size -= i->second.size();
      if (i == cur) {
  ...
  set<unsigned> high_queue_keys;
  set<unsigned> queue_keys;
  vector<SubQueue> high_queue;
  vector<SubQueue> queue;

  SubQueue *create_queue(unsigned priority) {
    typename set<unsigned>::iterator p = queue_keys.find(priority);
    if (p != queue_keys.end()) {
  ...
  void distribute_tokens(unsigned cost) {
    if (total_priority == 0)
      return;
    for (typename set<unsigned>::iterator i = queue_keys.begin();
	 i != queue_keys.end();
	 ++i) {
...
public:
  ...
  unsigned length() const {
    unsigned total = 0;
    for (typename set<unsigned>::const_iterator i = queue_keys.begin();
	 i != queue_keys.end();
	 ++i) {
      assert(queue[*i].length());
      total += queue[*i].length();
    }
    for (typename set<unsigned>::const_iterator i = high_queue_keys.begin();
  ...
  void remove_by_filter(F f, list<T> *removed = 0) {
    ...
    for (typename set<unsigned>::iterator i = high_queue_keys.begin();
	 i != high_queue_keys.end();
	 ) {
  ...
  void dump(Formatter *f) const {
    ...
    for (typename set<unsigned>::const_iterator p = high_queue_keys.begin();
    ...
    for (typename set<unsigned>::const_iterator p = queue_keys.begin();
	 p != queue_keys.end();
	 ++p) {
      f->open_object_section("subqueue");
      f->dump_int("priority", *p);
