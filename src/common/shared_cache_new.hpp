...
#ifndef CEPH_SHAREDCACHE_NEW_H
#define CEPH_SHAREDCACHE_NEW_H
...
private:
  ...
  class Cleanup {
  public:
    SharedLRUNew<K, V> *cache;
    K key;
    Cleanup(SharedLRUNew<K, V> *cache, K key) : cache(cache), key(key) {}
    void operator()(V *ptr) {
...
public:
  SharedLRUNew(CephContext *cct = NULL, size_t max_size = 20)
    : cct(cct), lock("SharedLRUNew::lock"), max_size(max_size), 
      size(0), waiting(0) {}
